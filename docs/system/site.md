...
| `.vitepress/ordered-list-plugin.ts` | Marks nested ordered lists `role="list"`, so they keep their list semantics once `custom.css` numbers them `1.1.`. See [Theme](#theme). |
...
| `package.json`, `package-lock.json` | Pinned dev dependencies: `vitepress ^1.6.4`, `markdown-it-container`, `gray-matter`, `yaml`, `ajv`, `wrangler ^4`, and the site's two typefaces, `@fontsource-variable/ibm-plex-sans` and `@fontsource/ibm-plex-mono` (both `^5`, OFL-1.1). Use `make install` (`npm ci`). There are no npm scripts: the Makefile is the only entry point. |
...
`.vitepress/theme/index.ts` extends the default theme, adds `custom.css` and the two typefaces, and runs two behaviours in the browser: collapsible sections scroll their header into place (`details-scroll.ts`), and clicking a picture shows it big (`lightbox.ts`). The CSS changes only colours (CSS variables), the typefaces, where the light/dark switch shows, the home hero's picture, the look of the course cards, the Course Overview cards and collapsible sections, how wide images are on desktop, the lightbox, the numbering of nested ordered lists, and the footer on pages with a sidebar. Keep it that way. Each behaviour is one delegated listener on `document`, in its own file, started from the theme's `setup()`; a new one is a new file, not a component. A custom layout or Vue component is a bigger step: record it in [decisions.md](./decisions.md) first.

- **Typefaces: IBM Plex Sans for the pages, IBM Plex Mono for code.** `theme/index.ts` imports both and `custom.css` points VitePress's two font variables at them (`--vp-font-family-base`, `--vp-font-family-mono`), which covers the pages, the navigation, the sidebar and the search box as well as code.
  - **Self-hosted**, from `@fontsource`, exactly as the default theme ships Inter: the files are part of the build, so no request leaves the site for a font, and nothing in the Terms' privacy section has to change.
  - **The Sans is the variable build** (`@fontsource-variable/ibm-plex-sans`, the `wght` axis, upright and italic): one file covers every weight the theme asks for — 400 for text, 500 for navigation, 600 for headings and bold, 700 for the hero title. Mono has no variable build, so it takes three static weights: 400, 400 italic (some syntax themes slant comments) and 600 (tokens they bold).
  - **A page downloads two files, about 60 KB** (Sans latin 46 KB, Mono latin 15 KB); italics and other subsets come only if a page uses them. Characters outside the latin subset (→, ✓, ⌘) come from the reader's own font, as they did under Inter.
  - **The Inter preload is swapped out.** VitePress hard-codes `<link rel="preload">` for Inter on every page whenever the default theme is in use, with no option to turn it off. `transformHtml` in `config.mts` replaces it with the Sans file, so the preload fetches a font the page actually uses. After a VitePress upgrade, check that the replacement still matches (see [known-issues.md](./known-issues.md)).
...
- **Images are at most 400px wide on desktop** (768px and wider). Most course images are phone screenshots taken on a 2x screen: 728px wide for a 364px-wide phone. At the full column width (about 688px) they showed at almost twice their real size, and a tall one filled the screen. On phones they still use the full page width.
- **A screenshot is centred in whatever holds it** — the page column, or the indent of the step it belongs to, so a picture under step 2.1 sits under step 2.1. The rule is `.vp-doc p > img:only-child`, which is every standalone picture the course writes (Markdown puts each one in its own paragraph) and leaves an image set inside a line of text alone.
- **A glossary word explains itself on hover** (`glossary-tooltip.ts`, with `glossary-link-plugin.ts` doing the build-time half). Hovering a link into the glossary — or tabbing to it — opens a small bubble with the term, its type and its definition; Escape, moving away, or scrolling closes it. Clicking still goes to the glossary page. A dotted underline in `custom.css` marks the words that have one.
  - **The definition travels on the link.** `glossary-link-plugin.ts` matches any link ending `glossary.md#<anchor>` (or `/glossary#<anchor>`, once VitePress has rewritten it) and puts `data-glossary`, `data-glossary-type` and `data-glossary-body` on it, the body being the description rendered from `glossary.yaml` as inline Markdown. Pages keep writing a plain relative link, so `course/` stays renderer-neutral and the link still works on GitHub and with JavaScript off. The whole glossary is 12KB and a page carries only the terms it links to, which is why this is inlined rather than fetched — a tooltip that waits for a request is a tooltip that flickers.
  - **A tap can't open it, by design.** VitePress registers its router on `window` with `{capture: true}`, so it routes before any listener the theme can add: a tap would open the bubble *and* leave the page. Touch readers get the glossary page instead, which says the same thing in full. Making a tap work needs one of two trades — wrapping glossary links in `.vp-raw` so the router ignores them (and every glossary click becomes a full page load), or stripping `href` while the bubble is open (and the link stops being a link for assistive tech). Neither is worth it for 33 links; revisit if the glossary grows a lot.
- **Call-outs carry an emoji.** A GitHub alert (`> [!NOTE]` and friends, [course-format.md](./course-format.md#markdown)) shows an emoji in front of its title, so a reader sees which of three kinds it is before reading a word:

  | Emoji | Alert types | Means |
  | --- | --- | --- |
  | 💡 | `NOTE`, `TIP`, `INFO` | Information. Worth knowing; nothing breaks if it's missed. |
  | ⚠️ | `WARNING` | A warning. Doing this wrong costs the reader time. |
  | ‼️ | `IMPORTANT`, `CAUTION`, `DANGER` | A serious warning: work, money, secrets or a working install can be lost. |

  `IMPORTANT` is purple in the default theme, which reads as "worth knowing". Here it marks something that breaks an install, so `custom.css` points `--vp-c-important-*` at `--vp-c-danger-*`: the same red background as a `CAUTION`, in both light and dark, because the `-soft` shade the background uses follows the switch. That sits with the `--vp-c-tip-*`/`--vp-c-note-*` indigo overrides, and keeps red meaning "serious" across every call-out.

  VitePress turns each alert into a custom block whose class is the type in lowercase, and marks it `.github-alert`, so `custom.css` styles those and nothing else. The emoji are `::before` content on `.custom-block-title`, written as CSS escapes (`\1f4a1`, `\26a0\fe0f`, `\203c\fe0f`) followed by a non-breaking space; the two warning emoji need their variation selector (`fe0f`) or they render as plain black glyphs. GitHub draws its own icons for the same alerts, so this is the site's half of a signal the Markdown already carries — don't type an emoji into the page.
- **Clicking a picture shows it big** (`lightbox.ts`), which is what makes the 400px cap safe for a wide screenshot of a terminal. Clicking the dimmed area around it, the close button, or pressing Escape puts it back; clicking the picture itself does nothing, so it can't be shut by accident while it's being read.
  - **The size it grows to** is the smallest of three widths: the screen less its padding; the picture's own pixels, but never under 720px, so a screenshot that is already near the 400px cap still gets bigger; and the width it can have without running off the top and bottom. `lightbox.ts` passes the picture's width and its width-to-height ratio to `custom.css` as `--aith-lightbox-width` and `--aith-lightbox-ratio`; the CSS does the arithmetic, so it follows a resize or a turned phone by itself.
  - **It measures 893×332 → 893 wide on a 1440px screen, 351 on a 390px phone**, a 728×1120 screenshot → 466×717 (limited by the height), and a 407×65 strip → 720×115 (grown). Re-measure these if the sizing changes.
  - **The picture carries `width` and `height` attributes.** A fresh `<img>` has no size until its bytes are decoded, even from the cache, so without them the overlay opens empty for a frame. They are a hint only: the CSS `width` overrides them, which is why the size comes from `width`, not `max-width`.
  - **Pictures inside a link or a `<summary>` are left alone**, as are middle-clicks and modified clicks (open in a new tab). The overlay sits at `z-index: 100`, over the sidebar (50) and the nav bar (20), and hides the page's scrollbar while it is open, padding the page by the scrollbar's width so nothing jumps.
...
- **Nested numbered lists show their whole number:** `1.`, then `1.1.`, then `1.1.1.`, with a trailing dot at every level and no limit on depth. Pages stay plain Markdown (`1.` at every level), so GitHub shows flat numbering and only the site numbers the levels. Authors never type the numbers (see [course-format.md](./course-format.md#markdown)).
  - **Only nested lists change.** A counter (`aith-step`) is reset on every `ol` and incremented on every `> li`, but nothing prints it at the top level: those items keep the browser's own marker, which already draws "1.". Single-level lists — nearly every list in the course — are untouched.
  - **`ordered-list-plugin.ts` marks the lists the CSS may number** with `class="sub-numbers"`, rather than the CSS matching every `ol ol`. That keeps three kinds of list out of it: a list whose numbers the browser should keep (one that starts at `4.`, where `counters()` can't read `start`), a list another rule owns (the Course Overview cards), and a list under a bullet, which has no parent number to carry — it starts again at `1.`, as GitHub shows it. A list inside a skipped list is skipped too. A list the rule never sees keeps its marker, so the failure is plain numbering, never no numbering.
  - **Not `::marker`,** which would be neater and would keep the markers. Safari supports only `color` and `font-size` on it (MDN browser-compat-data: "Safari support is limited to color and font-size"), so a Mac or iPhone would show a flat `1.`.
  - **`list-style: none` makes Safari drop the list from VoiceOver,** so `.vitepress/ordered-list-plugin.ts` marks every nested ordered list `role="list"`, the same fix as the Course Overview cards.
  - **The number sits in the item's own gutter** (`position: absolute` in a `padding-left`, set by `--aith-step-column`), so a line that wraps — or a picture under the step — lines up with the text above it, not under the number. The column is 2.75em at the second level, 3.6em at the third and 4.5em at the fourth: room for two digits at the top level and one below (`12.9.` is about 2.25em, `12.9.9.` about 3.1em). It is left-aligned, so the shared start of `1.9.` and `1.10.` stays in one place down the list, and `tabular-nums` keeps the digits even. A label wider than its column overlaps the text rather than pushing it, so keep nesting to three levels.
