...
- Course 001's `01-installations/04-make/` teaches Make, but no lesson uses it yet: the page says "Later in this course, you run the same few commands again and again", and NanoClaw itself has no `Makefile`. Either add the `make` shortcuts the later lessons are meant to use, or reword that page so it doesn't promise something the course never does.
