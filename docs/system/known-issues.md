...
| Every page downloads an Inter font file again | A VitePress upgrade changed the preload tag, so the replacement in `transformHtml` (`config.mts`) no longer matches and its hard-coded Inter preload is back. Nothing uses Inter, so it is a wasted download, not a visible fault. Fix the pattern against the new tag in `.vitepress/dist/index.html`. | [site.md](./site.md#theme) |
| Text falls back to the system font for a moment, or for the odd character | Expected. The fonts are self-hosted with `font-display: swap`, so text shows in the fallback until they arrive, and `→`, `✓` and `⌘` sit outside the latin subset and always come from the reader's own font. | [site.md](./site.md#theme) |
...
| A theme click handler can't stop VitePress following a link | Its router listens on `window` with `{capture: true}`, so it runs before anything on `document`, in either phase, and it calls `go()` behind an `await` — a later `preventDefault` can't stop what has already been scheduled. Check `.defaultPrevented` from a `window` capture listener before assuming your handler ran first. | [site.md](./site.md#theme) |
| Driving the site over CDP: a click or tap lands on the wrong element | The theme sets `scroll-behavior: smooth`, so `scrollIntoView` is still animating when the coordinates are read. Scroll with `behavior: 'instant'`, wait, then re-read `getBoundingClientRect` and assert `document.elementFromPoint` is the element before dispatching. | — |
| Under CDP mobile emulation, dispatched coordinates miss their target | The page scale factor applies, so CSS pixels from `getBoundingClientRect` aren't the input coordinates. Test the branch by overriding `matchMedia` on a desktop viewport instead. | — |
