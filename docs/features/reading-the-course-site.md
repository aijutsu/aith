...
- **Lessons**: each course is split into short pages, e.g. https://aith.aijutsu.dev/001-building-agents-with-nanoclaw/01-installations/. The sidebar lists them, and "Next page" at the bottom of each one leads to the next. A long lesson is split again into one page per step, e.g. https://aith.aijutsu.dev/001-building-agents-with-nanoclaw/01-installations/06-make/. The sidebar nests those under their lesson, and the lesson's own page is the overview, so a learner can stop after one step and come back to the next.
...
- **An About Aijutsu page** at https://aith.aijutsu.dev/about, in four parts: who Aijutsu is, what it does (three services across the AI, cloud, and compliance domains), why it runs this course, and who is on the team. It ends with how to contact Aijutsu. It's in the sidebar under "Reference".
- **Steps for your own computer.** Where the steps differ by operating system, the page has a section for each one (macOS, Windows, Linux). Only one is open at a time: opening one closes the others, so there's no wall of steps for systems you don't use. Opening or closing a section brings its title to the top of the screen, so the steps you picked start where you're looking.
- **Search** (top of every page). It works in the browser, with no outside search service, and it covers the glossary too.
- **Pictures enlarge.** Click a screenshot to see it big, over the page. Click the dimmed area around it, or press Escape, to go back.
